...
void ariadne(float *vertex,int *nrhits,float *position,
	     float *direction,float *goodness,float *quality,
	     float *cos_scat,int *nrscat);
void cone_intersect(float *axis1,float *axis2,float *dir1,float *dir2,
		    int &solution);
void find_largest_cluster(char *active,float *directions,int nrdir,float *clusdir,
			  float &limit,int &max_index,float &maxmag,int &nradd,
			  int &clus_index);
void refine_cluster(char *active,float *directions,int nrdir,float *clusdir,
		    float &limit,int &max_index,float &maxmag,int &nradd,
		    int &clus_index,float *cos_scat,int *nrscat);
void final_scan(char *active,float *directions,int nrdir,float *clusdir,
		float limit,int &max_index,float &maxmag,int &nradd,int &clus_index);
void sum_dir(float *directions,char *active,int nrdir,
	     float *sum,int &nradd,float *direc,float cos_min);
...
/*---------------------------------------------------------------------*
 ...
 ----------------------------------------------------------------------*
  ...
  char  *active;                                                     //*
  int   max_index,nradd,nrdir,clus_index;                            //*
  float distance,tempdir[4],*vector,*directions,*clusdir;            //*
  float limit,maxmag;                                                //*
/*                                                                     *
***********************************************************************/
  *goodness=-1;
  *quality=0.5*(*nrhits)*(*nrhits+1);
  //-------------------------------------------------------------------
  // get all the direction vectors from the vertex to the PMTs
  //-------------------------------------------------------------------
  vector=new float[*nrhits*3];
  for(row=0; row< *nrhits; row++)
    {
      distance=0;
      for(i=0; i<3; i++)
	{
	  vector[3*row+i]=positions[3*row+i]-vertex[i];
	  distance+=vector[3*row+i]*vector[3*row+i];
	}
      distance=sqrt(distance);
      for(i=0; i<3; i++)
	vector[3*row+i]/=distance;
    }
  ...
  for(row=0; row<*nrhits; row++)
    for(column=row+1; column<*nrhits; column++)
      {
	cone_intersect(vector+3*row,vector+3*column,
		       directions+4*nrdir,
  ...
  find_largest_cluster(active,directions,nrdir,clusdir,
		       limit,max_index,maxmag,nradd,clus_index);
  if (clus_index==0)
    {
      delete [](directions);
      delete [](active);
  ...
  refine_cluster(active,directions,nrdir,clusdir,
		 limit,max_index,maxmag,nradd,clus_index,cos_scat,nrscat);
  if (max_index==-1)
    {
      delete [](directions);
      delete [](active);
  ...
  final_scan(active,directions,nrdir,clusdir,limit,max_index,maxmag,nradd,clus_index);
  if (max_index==-1)
    {
      delete [](directions);
      delete [](active);
  ...
  *goodness=clusdir[4*(max_index-1)+3]/(*quality);
  sum_dir(directions,active,nrdir,tempdir,nradd,
		clusdir+4*(max_index-1),COS_CUT);
...
void printalldir_(float *directions,int *nrdir)
{
  ...
  printf("%d directions\n",*nrdir);
  for(i=0; i<*nrdir; i++)
    printf("%lf %lf %lf %lf\n",directions[4*i],directions[4*i+1],
	   directions[4*i+2],directions[4*i+3]);
}
 ...
 ***********************************************************************/
  ...
  solution=2;
  sin_alpha2=(ax1[0]*ax1[0]+ax1[1]*ax1[1]+ax1[2]*ax1[2])*
    (ax2[0]*ax2[0]+ax2[1]*ax2[1]+ax2[2]*ax2[2])-cos_alpha*cos_alpha;
  if (sin_alpha2==0)
    {
      solution=3;
      return;
			  ...
			  int &clus_index)
{
  ...
  for(row=0; row<nrdir; row++)
    if (active[row])
      {
	sum_dir(directions,active,nrdir,clusdir+clus_index*4,nradd,
		directions+4*row,COS_CUT);
	/*-------------------------------------------------------------------
  ...
  *nrscat=0;
  ...
  for(i=0; i<clus_index; i++)
    if (clusdir[3+4*i]>limit)
      {
	cosine=clusdir[4*i]*tempdir[0]+clusdir[4*i+1]*tempdir[1]+
	...
	sum_dir(directions,active,nrdir,tempdir,nradd,clusdir+4*i,COS_CUT);
	if (tempdir[3]<maxmag*CUT_FRAC2) clusdir[4*i+3]=0;
...
/***********************************************************************
 ...
 -------------------------------------------------------------------
  ...
  for(index=0; index<nrdir; index++)
    if (direc[0]*directions[4*index]+direc[1]*directions[4*index+1]+
	direc[2]*directions[4*index+2]>cos_min)
    {
      active[index]=0;
...
void printdir_(float *dir)
{
  printf("fortran: sum=%lf %lf %lf %lf\n",dir[0],dir[1],dir[2],dir[3]);
}
