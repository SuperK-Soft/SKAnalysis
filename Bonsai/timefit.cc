                              ...
                              float *norm_fact,float *time_range)
{
  ...
  // 2020/09/18 y.takeuchi: to avoid Segmentation fault
  //----------------------------------------------------
  if (!isfinite(tofmin) || !isfinite(tofmax)) {
      printf("timefit::sigrange  tofmin=%d  tofmax=%d\n",tofmin, tofmax);
      return;
  }
  //----------------------------------------------------
...
//----------------------------------
// 04-JUN-2019 fixed by Ed, Michael
//----------------------------------
