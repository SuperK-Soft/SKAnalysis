...
\DoxyCodeLine{LUN2 1                                         \# one or more additional LUNs may be allocated if required}
...
\item duplicate LUNs may be needed if invoking SKOFL/\+ATMPD functions that hard-\/code the LUN number, and have different hard-\/coded values.
...
\item Reading ROOT files can be sped up by only enabling branches you will use. To disable specific branches use\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{StartSkippedInputBranches}
\DoxyCodeLine{branchA}
\DoxyCodeLine{branchB}
\DoxyCodeLine{EndSkippedInputBranches}

\end{DoxyCode}

\item Alternatively to disable all branches other than those specified, use\+: 
\begin{DoxyCode}{0}
\DoxyCodeLine{StartActiveInputBranches}
\DoxyCodeLine{branchA}
\DoxyCodeLine{branchB}
\DoxyCodeLine{EndActiveInputBranches}

\end{DoxyCode}

\item this will disable all branches other than {\ttfamily branchA} and {\ttfamily branchB}.
\item for skroot files in {\ttfamily copy} mode, an output file will be created where entries can be copied straight from input to output.
\item Branches not desired in the output can be omitted from the copy by listing them in a similar fashion as above, using either
\item {\ttfamily Start/\+End\+Skipped\+Output\+Branches} or {\ttfamily Start/\+End\+Active\+Output\+Branches}. Branches disabled in the output but not the input
\item will be read in and accessible, but the output file will not contain them.
...
\item N.\+B. The minimum set of active input branches for calling lf\+\_\+allfit seems to be\+:
\item SOFTWARETRG
\item EVENTHEADER
\item PEDESTALS
\item TQLIST
