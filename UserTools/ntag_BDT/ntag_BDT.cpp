...
bool ntag_BDT::Initialise(std::string configfile, DataModel &data){
	...
	// whether to carry input branch data over to output
	// or just make an output with only the new info
	bool copy_over_data=false;
	m_variables.Get("copy_over_data",copy_over_data);
	if(copy_over_data){
		// make output tree with same branch structure as input tree
		treeout = myTreeReader->GetTree()->CloneTree(0);
	} else {
		treeout = new TTree("data","data");
		// to check alignment
		treeout->Branch("nevsk",&skhead_.nevsk);
	}
	
	// check if we have the 'type' or 'smearedvertex' branches (old relic analysis MC)
	got_type = (myTreeReader->GetTree()->GetBranch("type")!=nullptr);
	got_smeared_vtx = (myTreeReader->GetTree()->GetBranch("smearedvertex")!=nullptr);
	
	// make output branch arrays
	neutron5 = new float[MAX_EVENTS];
	nlow = new int[MAX_EVENTS];
	treeout->Branch("np",  &np);
...
bool ntag_BDT::Execute(){
	...
	// upstream SK2p2MeV Tool may not generate an output entry if there are no candidates.
	// To keep output trees 1:1 matched to inputs, we can detect this tracking the number of entries
	// and only applying the BDT when there is a new entry; otherwise filling output tree with 0 candidates.
	// XXX this only works when processing the Tree as it is being generated
	// FIXME add mechanism to detect whether we're reading from file or tree being generated by upstream Tool
	int n_in_entries = myTreeReader->GetEntries();
	if(n_in_entries==last_in_entries){
		Log(m_unique_name+": there is no new entry from upstream tools, number of candidates for this event is zero",v_debug,m_verbose);
		np=0;
	} else {
	...
	if(!passing_indices.size()){
		...
		np = 0;
		std::fill(neutron5, neutron5 + MAX_EVENTS, 0); // strictly overkill to fill out to MAX_EVENTS, np would be sufficient
	} else {
