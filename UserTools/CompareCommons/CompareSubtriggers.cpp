...
CompareSubtriggers::CompareSubtriggers():Tool(){}

bool CompareSubtriggers::Initialise(std::string configfile, DataModel &data){
  ...
  if (!ok || trigger_time_names.empty()){throw std::runtime_error("CompareSubtriggers::Execute - on trigger time variables name selected!");}
...
bool CompareSubtriggers::Execute(){
  ...
  if(!ok){throw std::runtime_error("CompareSubtriggers::Execute couldn't retrieve trigger times");}
  ...
  if(get_ok!=0){
    Log("CompareSubtriggers::Execute:: Error! skcread returned "+std::to_string(get_ok)
...
bool CompareSubtriggers::Finalise(){
  
  return true;
}

int CompareSubtriggers::GetReaderLUN(){
  std::string reader_name = "";
  m_variables.Get("reader_name", reader_name);
  if (m_data->Trees.count(reader_name) != 1){
    throw std::runtime_error("CompareSubtriggers::GetReaderLUN couldn't get the LUN of a valid TreeReader");
  }
  return m_data->GetLUN(reader_name);
}

void CompareSubtriggers::CompareSKTQZ(const sktqz_common& before, const skchnl_common& before_chnl,
...
template <typename T>
bool CompareSubtriggers::CompareArray(T* a, T* b, const int& N){
  bool match = true;
...
template <typename T>
void CompareSubtriggers::PrintPMTArray(T* a, const skchnl_common& a_chnl,
...
template <typename T>
void CompareSubtriggers::PrintArray(T* a, T* b){
  for (int i = 0; i < 5; ++i){
...
void CompareSubtriggers::Compare_skq(const skq_common& orig, const skchnl_common& origchnl, const skq_common& dupl, const skchnl_common& duplchnl) const {
  // nqisk:                                                                                                                                                                                                                                                                       
...
void CompareSubtriggers::Compare_skt(const skt_common& orig, const skchnl_common& origchnl, const skt_common& dupl, const skchnl_common& duplchnl) const {
  // timnsk:                                                                                                                                                                                                                                   
